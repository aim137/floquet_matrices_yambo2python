...
sed -i 's/-1.000000 |-1.000000 |/ 5.4332 | 5.4332 |/' $input_file  
...
sed -i 's/NLCorrelation= "IPA"/NLCorrelation= "SEX"/' $input_file
#sed -i 's/Xorder= 2/Xorder= 4/' $input_file
sed -i 's/HARRLvcs=.*.#/HARRLvcs= 2  Ha  #/' $input_file
sed -i 's/EXXRLvcs=.*.#/EXXRLvcs= 2  Ha  #/' $input_file
...
sed -i "s/FL_2D_ild=.*.#/FL_2D_ild= 0.0000001   #/g" $input_file # very small value to completely eliminate V_Hartree and focus on the self-energy

input_file=i_floptics
mv i2-fl-shg-$datetag $input_file

mpirun -np $SLURM_NTASKS $bins/yambo_fl -F $input_file -J "job$datetag,$COLLDIR" -C out_job$datetag -I ../../../
